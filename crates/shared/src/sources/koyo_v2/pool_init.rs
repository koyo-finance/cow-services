//! Koyo pool registry initialization.
//!
//! This module contains a component used to initialize Koyo pool registries
//! with existing data in order to reduce the "cold start" time of the service.

use super::graph_api::{KoyoSubgraphClient, RegisteredPools};
...
/// A Koyo pool registry initializer that always returns empty pools.
///
/// This can be used to index all pools from events instead of relying on the
/// Koyo subgraph for example.
...
#[async_trait::async_trait]
impl PoolInitializing for KoyoSubgraphClient {
    async fn initialize_pools(&self) -> Result<RegisteredPools> {
